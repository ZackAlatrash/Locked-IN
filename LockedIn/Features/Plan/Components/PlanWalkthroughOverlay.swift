...
struct PlanWalkthroughOverlay: View {
...
private extension PlanWalkthroughOverlay {
    ...
    var overlayContent: OverlayContent {
        ...
        case .planningIntro:
            return OverlayContent(
                ...
                message: "This is where you schedule your protocols for the week. Each session gets a day and a time slot.",
                continueTitle: "Next"
            )
        case .planningQueue:
            return OverlayContent(
                title: "Protocol Queue",
                message: "These are your unscheduled sessions. Tap one to arm it for placement, then tap any open slot on the board to drop it in.",
                continueTitle: "Next"
            )
        case .planningSelectProtocol:
            return OverlayContent(
                title: "Select a Protocol",
                message: "Tap the highlighted protocol card to select it, then tap any open slot on the board to place your session.",
                continueTitle: nil
            )
        case .planningSelectSlot:
            return OverlayContent(
                title: "Place It",
                message: "Tap any open slot on the board to schedule the session — morning, afternoon, or evening.",
                continueTitle: nil
            )
        case .planningPlacedConfirmation:
            return OverlayContent(
                title: "Session Scheduled",
                message: "That slot is locked in. The Regulator can auto-fill the rest of your week in one tap — let's set that up.",
                continueTitle: "Next"
            )
        case .planningRegulatorIntro:
            return OverlayContent(
                title: "The Regulator",
                message: "REGULATE automatically distributes your remaining unscheduled sessions across the week based on your time preferences.",
                continueTitle: "Next"
            )
        case .planningRunRegulator:
            return OverlayContent(
                title: "Run It",
                message: "Tap REGULATE to generate a plan for the rest of your week.",
                continueTitle: nil
            )
        case .planningCompleted:
            return OverlayContent(
                title: "Week Planned",
                message: "Your sessions are on the board. Head to Check-In when it's time to mark one complete.",
                continueTitle: "Done"
            )
        default:
            return OverlayContent(
    ...
    var calloutCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 10) {
                ...
                if let continueTitle = overlayContent.continueTitle {
    ...
    func spotlightMetrics(for frame: CGRect) -> (center: CGPoint, width: CGFloat, height: CGFloat, cornerRadius: CGFloat) {
        (
            ...
            cornerRadius: min(20, max(10, min(frame.width, frame.height) * 0.18))
        )
    }
}
