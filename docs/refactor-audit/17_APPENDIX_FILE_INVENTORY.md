# 17_APPENDIX_FILE_INVENTORY

## Purpose and Scope
Provide a working index of current Swift source files with categorized metadata for refactor planning.

## Summary
- Indexed Swift files: `102`
- Source of truth: `docs/refactor-audit/data/swift_path_index.csv` + `docs/refactor-audit/data/swift_loc_by_file.txt`
- Categories are heuristic and intended for navigation, not semantic proof.

## Inventory Table
Columns:
- `Path`: exact file path
- `Likely Feature`: folder-derived feature grouping
- `Likely Type/Category`: naming/path heuristic
- `Approx Responsibility`: inferred from folder + filename
- `File Size Note`: LOC-based bucket
- `Audit Note`: quick risk marker
...
## Notes
- This index intentionally includes files marked as potential stale/unused candidates so Pass B can resolve them explicitly.
- Size notes use Pass A thresholds (`>150`, `>300`, `>500`, `>1000` LOC).

## Index Quality Finding
### Finding 1: Responsibility/category fields are heuristic labels
- Type: Code smell
- Severity: Low
- Confidence: High confidence
- Evidence:
  - Table generation logic: `docs/refactor-audit/data/appendix_file_inventory_table.md` (derived from path and filename heuristics).
- Why this is risky now:
  - Some rows can misclassify hybrid files with mixed responsibilities.
- Pass B follow-up:
  - Correct category labels when dependency and behavior mapping is completed.

## Conclusion
This appendix is the working file map for Pass B dependency tracing and split planning.
